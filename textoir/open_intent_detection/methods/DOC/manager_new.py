        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(data.train_dataloader, desc="Iteration")):
                ...
                with torch.set_grad_enabled(True):
                    ...
                    label_ids = F.one_hot(label_ids, num_classes = label_nums).float()
                    _, logits = self.model(input_ids, segment_ids, input_mask, label_ids, loss_fct=loss_fct)
                    loss = F.binary_cross_entropy_with_logits(logits, label_ids)
